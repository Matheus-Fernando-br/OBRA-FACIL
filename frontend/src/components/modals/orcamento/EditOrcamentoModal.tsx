...
export function EditOrcamentoModal({
  ...
  const dataPublicacao = budget ? new Date(budget.data_publicacao) : new Date();
  const dataValidade =
    validade > 0
  ...
  useEffect(() => {
    ...
    setSelectedClient(budget.cliente._id);
    ...
    setLogradouro(budget.endereco.rua);
    ...
    setValidade(Number(budget.valido_durante));
  }, [budget, visible]);

  async function handleSave() {
    try {
      if (!budget || !token || !user) {
        ...
        <Pressable style={{ flex: 1 }} onPress={(e) => e.stopPropagation()}>
          <View style={globalStyles.addCard}>
            ...
            <View style={globalStyles.modalHeader}>
              ...
              <Text style={globalStyles.addTitle}>Editar Orçamento</Text>
            ...
            >
              ...
              >
                <Picker.Item label="Selecione um cliente" value=""                 style={{
                  padding: 15,
                  borderRadius: 10,
                  width: "100%",
                  backgroundColor: COLORS.backgroundSection,
